...
function generatePlan() {
  ...
  if (steps.length === 0) {
    ...
    html += '<div class="phase-header">Upgrade Plan by Hero</div>';
    for (const m of members) {
      ...
      if (skillChanges.length > 0) {
        ...
        if (allSame && skillChanges.length === a.skillsFrom.length) {
          changes.push(`<span class="badge badge-skill">All skills ${a.skillsFrom[0]} \u2192 ${a.skillsTo[0]} (+${a.skillsTo[0] - a.skillsFrom[0]})</span>`);
        } else {
      ...
      // Cost summary
      const costs = [];
      if (a.levelXP > 0) costs.push(fmt(a.levelXP) + ' XP');
      if (a.medalCost > 0) costs.push(fmtN(a.medalCost) + ' medals');
      if (a.shardCost > 0) costs.push(a.shardCost + ' shards');
      ...
      html += `<div class="hero-plan-card">
        ...
        ${a.pts > 0 ? `<div class="hero-plan-pts">+${fmtN(a.pts)} Thursday pts</div>` : ''}
      </div>`;
    }

    // Collapsible detailed steps
    html += `<details class="detail-steps"><summary>Show ${steps.length} detailed steps</summary>`;
